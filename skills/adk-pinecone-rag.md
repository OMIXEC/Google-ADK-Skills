---
name: adk-pinecone-rag
description: Build production RAG pipelines using Pinecone vector database with ADK agents. Actions include setup (create serverless index), ingest (documents to vectors with chunking), create_agent (RAG-enabled agent), and query (semantic search). Supports hybrid search, multi-tenant namespaces, reranking (Cohere), and persistent long-term memory for knowledge-based Q&A systems.
version: 1.0.0
---
