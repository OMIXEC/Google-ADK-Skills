...
def generate_python_edges(edges: list) -> str:
    ...
    for edge in edges:
        ...
        if condition:
            ...
            # ADK 2.3: routed edges are followed when the source node emits a
            # matching route value. Tag the branch and have `source` emit it.
            edge_strs.append(
                f"        Edge(from_node={source}, to_node={target}, "
                f"route=\"{target}\"),  # emit when: {condition}"
            )
        else:
            edge_strs.append(f"        Edge(from_node={source}, to_node={target}),")
...
def generate_python(cfg: dict) -> str:
    ...
    # Imports
    sections.append("""from google.adk import Agent, Workflow
...
""")
    ...
    # Workflow
    sections.append("# ── ADK 2.x Workflow ───────────────────────\n\n")
    chain = ", ".join(["START"] + agent_names)
    sections.append(f"""root_agent = Workflow(
    name="{name}_workflow",
    edges=[
        ({chain}),
    ],
...
__all__ = ["root_agent"] + {tool_names} + {agent_names}
""")
...
""")
    ...
    if tool_filter:
        lines.append("# Tool filter (allowlist): pass tool_filter to each McpToolset constructor above, e.g.")
        lines.append(f"#   McpToolset(connection_params=..., tool_filter={tool_filter})")
        lines.append("")
