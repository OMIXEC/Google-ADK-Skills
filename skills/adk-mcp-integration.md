---
name: adk-mcp-integration
description: Connect ADK agents to external tools via Model Context Protocol (MCP). Supports databases (SQLite/PostgreSQL), web search (Brave), APIs (GitHub/GitLab/Notion/Slack), Pinecone multimodal vector DB, and custom MCP servers. Uses McpToolset with StdioConnectionParams for automatic tool discovery. Configure via server type, connection params, API keys, and timeout.
version: 1.0.0
---
